...
confidence 0.7 (둘 다 CRITICAL). FN(놓침) 이 FP(오탐) 보다 훨씬 위험한 PII
이므로 보수적으로 emit. PARANOID/STRICT 모드는 0.7도 차단, PERMISSIVE 는 1.0만.

**Placeholder** — 체크섬은 통과하지만 실제 발급된 적 없는 *명백히 가짜* 패턴.
예: 사업자번호 `0000000000`, 여권 `M00000000`, 차량번호 뒷자리 `0000`. 체크섬
알고리즘만으로는 수학적으로 valid 하지만 도메인 지식상 실제 사용 안 됨 →
별도 거부. k-pii 는 카테고리별 placeholder 거부 룰을 갖춤.
